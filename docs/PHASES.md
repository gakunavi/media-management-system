...
| ~~**U72**~~ | ~~リダイレクタURLに記事IDを入れる~~ | ✅ **2026-07-24 完了**（テーマ v177 本番有効化）。記事ページの LINE 導線9箇所すべてに `-ART-XXX` が付いた。★検証: 10記事を**読者と同じ条件**で取得し、`article-cta` 2〜3件＋テンプレート側6件＝計10〜11件・二重付与0・カテゴリ/トップ/LPは未付与・JSON-LD の `sameAs` は不干渉。deploy 後に APO が古い版を返していたので157本をパージ済み。★MMS 側は cowork の4条件（①末尾の `ART-\d+` を切り出す ②設置場所に混ぜない ③ART無しは記事=未指定 ④二重付与は末尾1回のみ）を実クリックで検証済み。**④の検証で設置場所に `article-cta-ART-002` が残るバグを発見し修正**（末尾のARTを全部剥がす方式に変更）。桁数可変・小文字・多重も確認 |
...
| **U78** | **Cloudflare の課金内訳が未確定**（¥909/月） | 実測で確認できたのは「asset-support.co.jp を Cloudflare が配信しており APO が有効」までで、**909円が何の料金かは請求画面を見ないと分からない**（APO 月5ドル相当か、ドメイン登録か、他か）。★APO は HTML をエッジでキャッシュするため、**WordPress で301を直しても古い応答が残る**。2026-07-23 のピラー301ループにも絡んだ疑いがある |
| **U79** | **見直し予定日が1件も設定されていない**（10/10件） | 「続けるか止めるか」を決める日。これが無いと惰性で払い続けても検出できない。**各ツールの予定日は石井さんが決める**（画面の「続ける/止めるを判定する」から入力） |
| **U80** | **エックスサーバーの月額が未入力** | サイト本体（記事179本＋診断LP）のホスティング。**Cloudflare ¥909 より高いはずで、コスト最大の可能性がある**。プラン・月額とも未確認。石井さんに確認が必要 |
| **U81** | **コスト推移は 2026-07 から**（それ以前は未計測） | `tool-cost-snapshot-daily` を 2026-07-24 に開始。過去の月は遡って埋めない（埋めると「先月まで無料だった」という嘘の推移になる・§3）。★12ヶ月貯まるまで年間の増減は判定できない |
| **U82** | **ART-090 は判定不能が確定している** | 2026-07-03 の rewrite に対し、適用後の実測が**0日**。直近90日で `ContentQuery` に1行も無く、**検索結果に一度も表示されていない**。判定日を延ばしても測れないので、打ち手そのものを見直す対象。★「クリック0」ではなく「測れていない」（§4-73） |
| ~~**U84**~~ | ~~打ち手の型ごとの優先順位が決まっていない~~ | ✅ **2026-07-24 決定**（cowork 依頼7回答）。**枠制**を採用: 月10枠を リライト6・新規4、内部リンク/Threads は枠外。重み係数は導入しない。★根拠: 表示回数の単一ソートは新規記事を構造的に沈める（SERP不在＝表示0） |
...
| **U89b** | **ページの重さ（画像4.1MB）** ※2026-07-24 に**訂正**。当初「かなり重い」と報告したのは誤り | ★**最初の報告が誤っていた**（石井さん「直接サイトを開いたがそんなに時間かかっていない」→ 実測し直して確認）。PSI のモバイル値（LCP 10.6〜20.6秒）は**低速4G＋CPU4倍遅の模擬条件**であり、実際の体感ではない。**実ブラウザの実測: TTFB 19ms ／ DOM完了 696ms ／ 読込完了 1.39秒**（診断LP）。デスクトップ戦略の PSI も LCP 3.5秒／4.7秒。**遅くない。** ★ただし**重さ自体は事実**: 転送 4.83MB（解凍後5.72MB）のうち**画像が4.1MB**、HTML 820KB、CSS 744KB。速い回線では体感に出ないが、モバイル回線では効く。★**CrUX（実ユーザー）のデータが無い**ため、実際の訪問者がどうかは**分からない**（訪問数が少なくGoogleが集計できていない）。→ 判断材料が揃っていないので**いま急いで直す根拠は無い**。画像の最適化は効果が大きく副作用も小さいので、他の作業のついでに cowork へ回すのが妥当。★PerfGate は劣化の検知であって絶対値は見ない |
| **U90** | **JSON-LD の `sameAs` が計測用リダイレクタになっている**（2026-07-24・cowork 指摘 → 出所特定） | `"sameAs": ["https://collect.asset-support.co.jp/r/line/article-cta"]`。`sameAs` は**同一主体を指す公式プロフィール**を入れる項目で、送客用URLは用途違い。★**出所は MMS ではなくメディア事業部側**（cowork の見立てと違った）: `shared/brand/organization.yaml` も `bind-template.py` も**正しい値**（`https://lin.ee/szd8e1x`）を持っており、`.claude/scripts/lp-link-inject.py` の `track_line_links()` が **本文全体の `lin.ee` を無差別置換していて JSON-LD まで当たっていた**。★実測: 公開記事30本抜き取りで **sameAs を持つ25本が全部リダイレクタ**。★差分提案済み（href 限定に絞る＝そもそも書き換えない）。転送先 `MMS_LINK_DEST_LINE` と yaml の値が**一致**しているので差し替え先の調達は不要。既存記事の再生成は**cowork のフローに乗せる**（§8-1 WP書き込みはMMS API一本化・本文の正はWP）。依頼文は `docs/prompts/cowork-sameas-source.md` |
| **U91** | **`url_health.py` が JSON-LD の中のURLを見ていない** | 登録URLしか叩いていないため、**パンくずの404（U75）も sameAs の誤り（U90）も自動検出できなかった**。どちらも記事HTMLを手で読んで発見している。★「JSON-LD の中のURL（`BreadcrumbList.item` / `sameAs` / `Organization.url`）も叩いて検査する」を入れれば両方まとめて出る。実装は MMS 側で判断する |
| **U92** | **★JSON-LD 検査で新たに4種のバグを検出**（2026-07-24・U91 の初回実行） | 60本を検査して**7本で問題**。①**`@id` が `/blog/` を指し 404**（100本中**86本**。公開URLは `/media/`。エンティティ同定に使う識別子が実在しないURLになっている） ②**パンくずのリダイレクトループ**（ART-161・`jigyo-shokei-...` — ART-142 の301ループと同型） ③**パンくず404**（ART-072 → `/category/tax-reform-news/`。U75 の未対応分が実在を確認できた） ④**`contentUrl` の動画が404**（ART-024・ART-142。★ART-142 は `xxx.mp4` というプレースホルダが本番に残っている）。★どれも**記事HTMLを読まないと分からない**もので、`url_health.py`（登録URLのみ）では原理的に検出できなかった。→ `docs/prompts/cowork-jsonld-findings.md` |
...
| P1.9 | ✅ done | PerfGate（デプロイ前後PSI計測・劣化でブロック） | — |
