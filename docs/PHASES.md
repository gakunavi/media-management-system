...
| **U62** | **ヘッダ/フッタのLINEリンクが常に `hp-` 計上**（2026-07-23） | `header-media` / `footer-media` はメディア面にも出るため、記事を読んでいる最中のクリックがHPに計上される。ページ文脈で接頭辞を出し分ける依頼を cowork に出した（本番反映待ち） |
| **U63** | **LINEの遷移先URLが未確定** | MMS の `MMS_LINK_DEST_LINE` は `lin.ee/5NVLBXA`。cowork の記録では承認済みは `szd8e1x`。★どちらが正か石井さんの確認待ち。**env 1箇所で全19本が切り替わる** |
