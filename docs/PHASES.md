...
| **U86** | **内部リンクの提案が1件しか出ていない**（過小） | cowork: 「クラスター→所属ピラーへのリンクが2本未満の記事」を機械的に列挙する提案ロジックを追加すべき。運用順序は「①head KW20位以内 → ②内部リンクでPillar押上げ → ③GEO再計測」で、②が未実装。個別効果の分離測定は諦め、**Pillar側の表示回数・順位を月次で見る集約KPI**で判定する |
