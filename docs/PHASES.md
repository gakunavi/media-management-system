...
| **U89b** | **★サイトが実測でかなり重い**（2026-07-24 PSI 初計測） | 4対象すべてで **LCP 10.6〜20.6秒**（モバイル・Googleの「良好」は2.5秒以下）。総バイト **2.2〜6.0MB**／リクエスト 60〜104件。★特に **診断LP（総合窓口）が 6.0MB** で最も重い — **転換の要のページ**。TTFB は 1〜44ms と速いので、遅さは**サーバーではなくページの中身**（画像・スクリプト・DOM量。記事の全高は実測 285,922px）。★**CrUX（実ユーザー）のデータは無い**ため、いま言えるのはラボ値（低速4G相当のシミュレーション）まで。実ユーザーの体感はこれより速い可能性があるが、**6MB という重さ自体は条件に関係なく事実**。★PerfGate は**劣化の検知**であって絶対値の良し悪しは見ない。この重さは別途 cowork と改善方針を決める必要がある（優先度は石井さんの判断） |
