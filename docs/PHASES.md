...
| **U77** | **worker が `idle in transaction` の接続を残す** | 2026-07-24、`ALTER TABLE "Job"` が4分以上ロック待ちで固まり、後続の全クエリが詰まった。原因は worker のジョブ取得クエリがトランザクションを開いたまま idle になること（ポーリング間隔20秒）。当面は **DDL 前に worker を止める**（§4-50）で回避しているが、**worker 側でトランザクションを閉じるのが本筋** |
