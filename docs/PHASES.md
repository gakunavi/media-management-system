...
| **U71** | **買い手軸で判定できない記事が40件** | 制度横断・税制改正・事業者向けなど、タイトルだけでは読者の予算規模が決まらないもの（例:「防衛特別法人税とは？」「節税と内部留保のバランス」）。ルールを増やすより、**実測（どの予算帯のリードが来たか）が貯まってから決める**方が正しい。段3の注記に「実際の適合率はこれより高い可能性」と出している |
| **U70** | **記事からの送客が記事別に分解できない** | リダイレクタの送り元が設置場所ID（`media-article-bottom`）で記事IDを持たない。記事別に出すには `/r/line/{設置場所}-{記事ID}` が要る。それまで `/content` は設置場所別の合計だけを出す（記事別に0を並べると「踏まれていない」と誤読される） |
