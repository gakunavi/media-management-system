...
| **U74** | ⚠️ **`/media/category/tax-reform/` が404**（2026-07-24・WP修正で新たに発生） | `tax-reform-news` term の削除に伴い、残っていた Redirection **id=25**（`/media/category/tax-reform/` → 削除済みterm・54ヒット）が404を返すようになった。**全記事のナビゲーションから貼られている**唯一の404カテゴリ（他7カテゴリは200）。id=85 も転送先が同じく死んでいる。**MMS からの修正は権限分類器にブロックされたため cowork へ差し戻し** |
...
| **U76** | **MMS 登録URLのうち26件が404**（`url_health.py` 初回実行） | 内訳: `LEGACY-*`（過去の実測があるだけの旧URL）8件・`PAGE-*`（GSC由来のサイトページ）15件・その他3件。**記事本体の404は0件**。ただし `PAGE-/media/)という中小企業向けの…` のようにURL欄に本文が混入した行が3件あり、過去の取り込みの不具合。**集計には出ないが掃除が要る** |
