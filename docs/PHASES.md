...
| **U69** | 記事のタグ付け（買い手軸・ファネル段階・メインKW・クラスタ・鮮度） | ⏳ **半分解決（2026-07-23・P4.9）**。買い手軸 139/179・ファネル段階 137/179 を `scripts/tag-buyer-axis.py` で一括付与し、**段3が実測で動くようになった**。★残り: メインKW 0・クラスタ 0・鮮度 0（それぞれ P4.5 / P4.3 / P3.5）。判定できなかった40記事は**推測で埋めず unknown のまま**（埋めると段3が嘘の数字になる）。`/content?filter=untagged` で一覧できる |
...
| **U73** | 🚨 **ART-142（P1ピラー）が本番で301の無限ループ**（2026-07-23 発見） | `keieikyoka-zeisei-complete-guide-2026` ⇄ `chushokigyo-keiei-kyouka-zeisei` が相互に301。読者もクローラも到達不能（表示246・クリック0）。**canonical は `keieikyoka-…`（ART-142）で確定**（生きている実体・内部被リンク13本・全台帳が一致）。修正は WP の Redirection 管理で、①本体の→chushokigyo ルールを削除 ②chushokigyo→keieikyoka の一方向のみ ③`-pillar` の転送先を `?post_type=blog&p=35861` から正規URLへ。**MMS からは適用できない**（WP REST が 403）。石井さんの wp-admin 作業か、cowork への WP 接続付与が要る |
| **U74** | **`/media/category/tax-reform-news/` が0本なのは記事不足ではなく term の不一致** | config 上このカテゴリを指定している記事は**14本ある**（ART-066〜073・162 ほか）。GNav のリンク先 slug `tax-reform-news` と、記事が実際に属する WP term（ID 11 / 21626・別slug）が食い違っている。ART-105 の `-2` slug・ピラーのループと同じ **slug/term 不一致系**。WP 上の実 term 確認が要る |
