...
> ★2026-07-24 に `stop_tracking_tag` を追加して12値（P2.11）。計測タグの発火が暴走したときの
> 緊急停止で、**他の値と性質が違う**（記事を良くする打ち手ではなくサイトを守る操作）。
> 過去の TTFB スパイク事故で本当に問題だったのは遅さではなく「何千回発火しても
> 誰も気づかず、止める手段も無かった」こと。監視だけでなく止める手段まで要る。
...
| `stop_tracking_tag` | サイト速度の保護 | **—** | **承認で即停止**（受口が受信を止める） |
...
| `free` | 無料プラン |
| `shared` | **自社が既に持っているもの**（2026-07-24 追加）。★`free` と分ける。エックスサーバーは無料ではなく**会社が元々払っている**もので、メディアのための追加コストが無いだけ。コスパを判定するのは**追加コスト**であって総支出ではない。ただし止まればサイトごと落ちるので、依存関係としては一覧に残す |
