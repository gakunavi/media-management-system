...
| | |
|---|---|
| 対象アカウント | `@898ubeoo`（【公式】中小企業のための節税総研）。★`lin.ee/5NVLBXA` と `lin.ee/szd8e1x` は**同じアカウントの短縮URLが2本あるだけ**で、配信先は同一（2026-07-23 実測確認） |
...
| 固定 `media-`（常にメディア面） | 9 | `media-entry-card` / `media-news-subscribe` / `media-lead-card` / `media-fixed-bar` / `media-category-cta-card` / `media-category-fixed-bar` / `media-category-corp-cta-card` / `media-category-corp-fixed-bar` / `media-tag-cta` |
