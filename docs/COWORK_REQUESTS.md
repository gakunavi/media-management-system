...
## 7【追加】診断LPのABCテストを畳んで1本にする

MMS が GA4 を直接読むようにして分かった実測（直近30日）。

```
                 実人数   イベント数
setsuzei-diagnosis-a    1人      1
setsuzei-diagnosis-b    4人     11
setsuzei-diagnosis-c    0人      0
                 ────────────────
                 合計    5人     12

lp_form_submit … 30日間で1件も発生していない
最後の到達    … 2026-07-11（以降11日間ゼロ）
```

**イベント12件は5人の再訪**だった。b の11イベントは4人。

`lp-ab-weekly-report.py` の勝者判定は「各パターン 到達100・送信10」。
月5人を3分割している限り、**この条件は構造的に満たされない**。
「データ蓄積中」ではなく「蓄積し終わる見込みが無い」状態。

これは設計書 §3.7.0 が既に明文化していた通り:
...
> | 母数の段階 | 主に使う手法 |
> |---|---|
> | **今（LP訪問 〜100/月）** | ①定性（Clarity録画・ヒアリング） ②**大きく変えて before/after** ③クラスタ単位 |
> | 先（1,000+/月） | ⑤ LP/CTA の A/Bテスト |
>
> 今の段階で小さなCTA文言のA/Bを回すのは時間の無駄。

設計時点の想定は月38訪問。**実測はその1/7の月5人**なので、条件はさらに厳しい。

### 依頼

1. **`lp-link-inject.py` の振り分けJS（`ROTATION_JS`）を廃し、LPを1本に固定する**
   - どれを残すかは**データからは決められない**（a=1人・b=4人・c=0人は差ではなく偶然）。
     デザイン意図で選んでほしい。判断材料が無ければ a（イラスト・初出）を残す
   - 残す1本に既存記事のリンクを寄せる
2. **`lp-ab-weekly-report.py` を before/after 型に組み替える**
   - パターン別の勝敗判定をやめ、「LPを大きく変えた前後28日」で比較する形へ
   - 判定条件（到達100・送信10）は据え置きでよい。1本に集約すれば3倍の速さで貯まる
3. **記事側の導線を先に直す**
   - クリック率 0.45% < 目標 0.5%（同スクリプトの診断ロジック1番）
   - LPを何本にしても、記事から人が来なければ判定母数は増えない
4. **`lp_form_submit` が1件も無い理由の確認**
   - LPまで来た5人が全員離脱したのか、イベント自体が発火していないのか
   - CF7 フォームID 601652 の送信計測が生きているか確認してほしい
   - ★イベントが壊れているなら、CVR 0% は「LPが悪い」ではなく「測れていない」

### MMS 側の状態

- `ga4-fetch-daily`（毎日07:30）が GA4 から直接取得。Notion 経由はやめた
- 段2ファネルに実測が入った: 表示4,556 → クリック154 → LP到達12 → 送信 未計測
- ABC を1本にしても MMS 側の変更は不要（`lp_view_<変種>` の変種が減るだけ）
