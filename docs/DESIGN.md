...
> **対象スコープ（2026-07-20 石井確定）: メディア／SNS運用。**
> 全社の手残り・他事業のP/L・時給管理は**対象外**（将来の拡張余地としてデータモデルに `Business` 軸だけ持たせる）。
>
> **★目的の整理（2026-07-22〜23 石井さんと確定・以降はこちらが正）**
>
> 施策（メディア・Threads・LP・公式LINE）はすべて **「見込み客募集」か「代理店募集」** の
> ために動いている。**各施策のゴールは問い合わせ増加**で、PV・クリック・登録は
> そのための手前の数字。MMS はそれを可視化して PDCA を回すためにある。
>
> 構造は **送客（HP／メディア／記事／Threads）→ 受け皿（7つ）→ リード → 成約**。
> 獲得しているものは **見込み客** と **代理店見込み** の2種類で、経路が違うだけ。
> 種別は**軸であって画面ではない**（→ §9-D21 / D23）。
>
> ★旧記述「獲得3ゴール（①直客 ②代理店 ③LINE登録）」はこの整理に置き換わった。
> LINE登録は受け皿への到達であって問い合わせではないため、ゴールに合算しない（§9-D21）。

| 役割 | 内容 |
|---|---|
| **① 獲得の可視化** | **問い合わせ件数（ゴール）と受け皿別の内訳**、そこに至るファネル全段を1画面に |
...
**仮説**
> 問い合わせ（見込み客・代理店見込み）が伸びない原因は「何が効いているか見えないこと」であり、
...
model ToolCostMonthly {   // ★ツールの費用を月次で残す。2026-07-24 追加
  id, businessId, period "YYYY-MM", toolId?
  monthlyYen?              // 固定月額
  variableYen?, variableAmount?, variableCurrency?, balanceSnapshot?  // 変動（使った分）
  state
}
// ★固定と変動を分ける: 合計だけでは「使っていないのに高い」と「使った結果高い」が
//   区別できず、打ち手が正反対になる（前者は止める、後者は成果次第）。
// ★変動費は前払い/従量に契約月額が無いため「残高の減り」から積む。補充時は消費0。
//   円換算の為替（MMS_USD_JPY）は仮定なので、原単位と併せて保存し画面にも出す。
// ★なぜ要るか: 「今いくら払っているか」は分かるが、増えているのか減っているのかが
//   分からない。ツールは足すのは簡単で止めるのは忘れるので、合計は放っておくと単調に増える。
// ★過去は埋めない（§3）。計測開始（2026-07）より前の月は「未計測」であって0円ではない。
//   遡ると「先月まで無料だった」という嘘の推移になる。
// ★検討中・トライアルも記録し、合計とは分けて出す（全部契約したらいくらか）。

model UrlHealthCheck {    // ★登録URLが本当に生きているか。2026-07-24 追加
  id, contentItemId?, url
  finalStatus, hops, loop Boolean, chain Json
  checkedAt Date
}
// ★なぜ台帳の突合では足りないか: 2026-07-23 に P1ピラー(ART-142)が本番で
//   301の無限ループに陥り到達不能だったが、台帳・config・MMS の三者は一致して
//   いたため静的な突合では一切引っかからなかった（表示246・クリック0 だけが残った）。
//   3日で3件（ピラーのループ／カテゴリ tax-reform の404／breadcrumb の404）。
// ★合格条件は「最大1ホップで200・ループ無し」。2ホップ以上は評価が減衰する。
// ★静的キャッシュ（Xserver）が古い301を返し続けるため、クエリを足して迂回する。
// ★301元・下書き・プレビューURLは対象外（異常が出続けると誰も見なくなる）。

model ContentQuery {      // ★記事が実際に何で検索されて表示されたか。2026-07-23 追加
  id, contentItemId, query
  clicks, impressions, ctr, position
  periodStart Date, periodEnd Date      // 集計期間（日次にしない）
}
// ★なぜ要るか: 179記事すべてで mainKeyword が空だったのは、埋める材料が
//   どこにも無かったから。gsc_daily.py は ["date"] と ["page","date"] しか
//   取っておらず、**どの検索語で来ているか**を一度も保存していなかった。
// ★Keyword（ラッコの調査KW＝狙い）とは別物。こちらは実測で、両者はふつう
//   一致しない。そのズレ自体がリライトの対象になる。
// ★日次にしない理由: page×query の日次は行数が跳ね、大半は0〜1表示のノイズ。
//   GSC 側も日次×query では閾値で行が落ちる。期間の集計スナップショットを週次で持つ。
// ★position は期間平均（GSCの定義）。加重平均ではないので合算してはいけない。

model AioCitation {       // ★AI検索（ChatGPT）に誰が引用されたか。2026-07-23 追加
  id, contentItemId, engine, date
  citedDomains String[], citedCompetitors String[], hasPrivateCompetitor Boolean
}
// ★ContentMetric に入れない理由: (contentItemId, metric, date) → Float の器に
//   ドメイン名を metric 文字列として埋めると名前空間が無制限に肥大する
//   （aio_competitor_moneyforward … のように増え続ける）。
// ★ChatGPT だけを保存する理由: 実測（2026-05〜06 の3353試行）で Gemini は
//   自社0% / 競合0.2% と、特定サイトを引用する挙動をほぼ持たない。
// ★いまは保存だけ。画面もアラートも作らない。現状 ChatGPT の被引用は
//   中小企業庁など公的機関が中心で民間競合には負けていない。
//   **民間競合が現れた時点**が打ち手の分岐なので、後から辿れるよう生データを残す。
...
#### 3.8.6-b 記事からの送客は記事別に分解できない（2026-07-23）

リダイレクタ（`/r/{dest}/{送り元}`）の送り元は**設置場所ID**（`media-article-bottom` 等）で、
**どの記事から踏まれたかを持たない**。記事末CTAは全記事で同じIDを使うため、
「記事Aから何人LINEへ行ったか」は出せない。

記事別に出すには `/r/line/{設置場所}-{記事ID}` のように記事IDをURLへ入れる必要がある
（`SAFE_SOURCE = /^[A-Za-z0-9_-]{1,40}$/` の40字に収まる）。

★それまでは **記事別に 0 を並べない**。0 を出すと「その記事は誰も踏んでいない」に見えるが、
実際は「記事別に測っていない」。`/content` では設置場所別の合計だけを出している。
...
### 4.1 `/` — ダッシュボード（石井さんが毎日見る画面）

**構成（2026-07-22 改訂）**: ゴールは**問い合わせ数を増やすこと**。だから一番上は問い合わせ件数で、
PV・クリック・表示はその手前の数字。構造は §3.8.3 の **送客 → 受け皿 → リード → 成約** に合わせる。

**表示期間**: 画面右上で切り替える（7日 / 28日 / 90日 / 今月 / 先月 / 任意区間）。既定は今月。
**全ての集計に同じ `since`/`until` を渡す**（`lib/period.ts`）。月次目標との比較は、
期間がちょうど1暦月のときだけ行う（docs/RULES.md §4-7 / §4-8）。

**3タブ構成** — 毎日見るもの（結果）と週に一度でよいもの（経路の計装状況）を同じ密度で並べると、
毎日見る数字が埋もれるため分けた。赤の警告だけは全タブ共通で上部に出す。

#### タブ1「結果」

**段1: 結果（問い合わせ）**
```
問い合わせ（ゴール）  5 / 2件   前期間比 +2      ← LINE登録は含めない
成約                  1件 ¥4,800,000  前期間比 +1
LINE登録              0件（問い合わせの手前・合算しない）

受け皿別  受け皿        問い合わせ  前期間  登録  成約  金額         計測
          HPの問い合わせ     2        0     —     1   ¥4,800,000   計測中
          診断LP        —(未計測)     —     —     —      —         未計測
          公式LINE           1        1     0     —      —         計測中
          Threads DM         2        2     —     —      —         計測中
```
※ 計測開始前は「—(未計測)」とし、0とは書かない（§3 規約）

**段2: ファネル（経路の階段）**
```
表示 → クリック → CTA表示 → CTAクリック → LP到達 → LP内CTA → 送信
                    ↑ 最大ドロップは、率を出せた区間が2つ以上あるときだけ名指しする
```
各データ源の最終取得日（GSC 〜7/20 ／ GA4 〜7/11）を併記する。

**推移**: 問い合わせ（ゴール）／平均掲載順位／送客の量（表示・クリック・PV）。
欠測は0で繋がず線を切る。末尾の反映待ちは欠測と分けて書く。

**段5: 次の一手** — `[承認][却下][差戻し]`。押すと Server Action で即 jobs 投入
**段3: 買い手の質**（2026-07-23 実装。P4.9 のタグ付けが前提だった） — 買い手軸タグの付与状況

#### タブ2「経路」

- **送客**（メディア／記事／Threads／HP）の量と、受け皿への到達量
- **送客 × 受け皿のマトリクス** — 目的は「どのマスが測れていないか」を出すこと。
  **未計測（直せる）と測定不能（直せない）を分ける**（§3.8.3）
- 次に計装すべき受け皿の一覧と、各詳細画面（/threads・/line・/lp）への導線

#### タブ3「健全性」

**段7: ジョブ健全性・配信・欠測** — 次の4つを**別々に**出す。ジョブが緑であることと、
そのジョブが書くはずの数字が入っていることは別（実際に全ジョブ success のまま pv が9日止まった）。

1. 検索データ（GSC）の欠測
2. Threads の配信が続いているか（キュー残数を含む）
3. Threads 実績（Insights）の回収が生きているか
4. 指標の鮮度 — 期待間隔は**その指標自身の履歴の中央値**から求め、3倍を超えたら警告。
   どの画面からも参照されていない指標は「未使用（貯めているだけ）」と出す

**段4「今週の変化」・段6「施策の生死」** は P4/P8 で追加する。

### 4.2 その他の画面

> **2026-07-23 改訂。** メニューは「獲得 / コンテンツ / 分析 / 運用」の4群。
> 獲得の並びは **結果 → 受け皿・送客元**（リード統計 → HPの問い合わせ → 電話 → LP → Threads → 公式LINE）。
> 受け皿ごとに画面を持つのは、①見込み客と代理店見込みのどちらが多いかを判断する
> ②その場で記録する ③期間を切って見る の3つが、合計値の1行ではできないため。

| 画面 | 内容 |
|---|---|
| `/leads` | **リード統計**。種別タブ（総合 / 見込み客 / 代理店見込み）・受け皿別・**きっかけ別**（`Lead.origin`）・成約・金額。代理店見込みタブに選別パイプライン（DM受信→有効→契約）と記録UI |
| `/hp` | **HPの問い合わせ**（＝ info メール。フォーム送信が info@ に届くだけで別経路ではない・2026-07-23 訂正）。種別・きっかけ・推移・一覧・「＋記録する」 |
| `/phone` | **電話**。手入力。★受電時に「何を見てお電話いただきましたか」を聞いて `Lead.origin` に記録する。これが唯一の経路情報 |
| `/lp` | **LP台帳**（§3.8.6）。一覧（種別・状態・到達・問い合わせ・CVR・リード・成約・A/B・最終データ）＋手入力での登録。LPは今後増える前提で、画面に直書きしない |
| `/lp/[slug]` | LP個別。階段（到達→CTA→問い合わせ→リード→成約）・A/B・**配布コード別の稼働**（そのLPの属性）・推移 |
| `/line` | **公式LINE**。階段は **登録 → 問い合わせ → 成約** の3段（2026-07-23）。送客（クリック）は計測系が違うので階段に載せず「入口」（HP / メディア / Threads / 不明）として別枠。友だち数は Messaging API から日次取得（webhook では総数が取れない）。★階段は種別で割らない（登録時点では見込み客か代理店見込みか分からず分母が割れない） |
| `/content` | **記事・投稿一覧**（2026-07-23 改訂）。★ゴールは問い合わせで、PV・クリック・順位はその手前。**「問い合わせ」列（`firstTouchContentId`）を最初に置く**。絞り込み（実測あり/なし・順位が落ちた・未タグ・ピラー）と並べ替え。**タグ付けの充足を画面に出す**（買い手軸・ファネル段階・KW・クラスタ・鮮度・AIO。埋まっていないと列が無く、空であることすら見えなかった）。記事からの送客は**設置場所別のみ**で記事別には分解できない（下記） |
| `/content/[externalId]` | 記事詳細。**この記事から生まれた問い合わせ**・メタ（未入力は警告色）・期間内の日次推移 |
...
| `/ideas` | ネタ（5供給源から自動起票） |
| ~~`/agency`~~ | **廃止（2026-07-23 石井さん）。** 獲得しているのは「見込み客」と「代理店見込み」の2種類で、経路が違うだけ。専用画面を持つと同じ数字が2箇所に散る。中身の行き先: 選別（DM受信→有効→契約）は `/leads` の「代理店見込み」タブ／アングル別のDM実績は `/threads`（投稿の効き）／配布コードの稼働は `/lp/[slug]`（そのLPの属性のため） |
| `/threads` | **Threads のゴールは並列に2つ（2026-07-23 石井さん確定）: ①メディア送客 ②DM（問い合わせ）**。1本の階段にしない（狙いが違う投稿を同じ物差しで測ると、効いている型を落とす）。投稿がどちらを狙ったかは**貼ったリンク**で判定する（`/r/soken/`→①・`/r/lp/`・`/r/line/`→副次・リンク無し＋代理店候補→②）。タブは「ゴール／投稿の効き／配信」。公式LINEへの送客はクリックまで実測できるが、**登録の投稿別帰属は測定不能**（follow イベントに経路が載らない）ため別枠 |
| `/experiments` | 施策管理（仮説・成功指標・撤退条件） |
| `/costs` | ツールのコスト・残高（§7）。残高不足は段7で赤 |
| `/jobs` | ジョブ監視・**指標の鮮度**（その指標自身の履歴から期待間隔を推定し、中央値の3倍で警告） |
