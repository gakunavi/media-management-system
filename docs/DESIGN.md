...
| Phase | 内容 | 依存 | 見積 |
|---|---|---|---|
| **P3.3** | **`IndexStatus`**（GSC URL Inspection API・未インデックス検知） | P1 | 0.5 |
| **P3.4** | `PageExperience`（CWV / PSI・モバイル別） | P1 | 0.5 |
| **P4.2** | **`LeadTouchpoint`**（マルチタッチ・アシスト貢献） | P2.5 | 1 |
| **P6.9** | **`Backlink` / `DomainAuthority`**（DataForSEO・自社＋競合） | P6.7 | 1 |
| **P5.8** | `trafficSource` / `aiEngine` の判別（AI検索流入） | P2.5 | 0.5 |
| **P8.2** | `ContentLifecycle` / `UrlRedirect` ＋ プルーニング自動起票 | P4.3 | 1 |
| **P4.11** | `ProductionCost`（記事別ROI） | P4 | 0.5 |
...
| Phase | 内容 | 依存 | 見積 |
|---|---|---|---|
| **P2.8** | **Microsoft Clarity 導入**（タグ設置＋フォームマスキング設定） | — | **0.1** |
| **P4.12** | **SEOスプリットテスト**（`SplitTest` / `SplitAssignment`・層別ランダム化・判定） | P4.3 | 0.5 |
| **P7.7** | LP/CTA A/Bテスト（`Experimentation` / `Variant`・サンプル数計算・underpowered拒否） | P2.5 | 1 |
| — | ※ `Experimentation` のモデルとセッション側の群割当は **P2.5 に含めて先に作る**（後入れ不可） | | +0.2 |
...
| Phase | 内容 | 依存 | 見積 |
|---|---|---|---|
| **P2.9** | `LandingPage` / `LpVersion` ＋ 既存LP投入 | P2.5 | 0.5 |
| **P2.10** | 電話CTAクリック計測 ＋ `/leads` 手動登録UI ＋ `sourceType` | P2.6 | 0.3 |
| **P3.8** | `RegulatoryEvent`（税制改正カレンダー）＋ 60日前の自動起票 | P1 | 0.5 |
| **P4.13** | `SeasonalityIndex`（季節調整）＋ 段1・段4への併記 | P4.10 | 0.5 |
| **P5.9** | `SnsAccountHealth` / `PostSchedule` / `CrossPromotion` | P5 | 1 |
| **P6.10** | m2 連携（`Lead` ⇄ m2 Deal・成約結果の還流） | P2.6 | 1 |
...
| Phase | 内容 | 依存 | 見積 |
|---|---|---|---|
| **P1.8** | **WP書き込みのMMS一本化**（`wp-publish.py` → API化）＋日次ハッシュ突合 | P1 | 1 |
| **P3.9** | `UptimeCheck`（5分間隔・フォーム含む・連続3回失敗で通知） | P0 | 0.3 |
| **P4.14** | `GenerationProvenance`（スキル/モデルのバージョン記録） | P4 | 0.3 |
| **P6.11** | `LinkCheck`（週次・tier1優先） | P4.3 | 0.5 |
| **P8.3** | `BrandMention`（レピュテーション監視） | P5 | 0.5 |
...
| Phase | 内容 | 依存 | 見積 |
|---|---|---|---|
| **P2.5に内包** | 計測タグの設計原則（§3.10.3）を実装規約として適用 ＋ `/api/ingest` のレート制限 | — | +0.3 |
| **P2.11** | **`TelemetryVolume`（発火回数監視）＋閾値アラート＋ワンクリック停止** | P2.5 | 0.5 |
| **P1.9** | `PerfGate`（デプロイ前後のPSI計測・劣化でブロック） | P0 | 0.5 |
| **P3.10** | `Incident` ＋ 過去5件の登録 | P0 | 0.3 |
...
4. 競合ギャップ（`CompetitorSnapshot` で自社圏外のKW）
...
> **推奨: M-A（7日）で一度止めて実際に使う。** 計測が始まればデータが溜まり始め、以降の設計判断が実測ベースになる。
...
| Phase | 内容 | 依存 | 見積 |
|---|---|---|---|
| **P0.5** | **個人情報・同意対応**（ポリシー改定案・同意チェックボックス・`ConsentRecord`・保存期間ジョブ）＋**専門家確認** | — | 0.5 |
| **P0.7** | バックアップ3箇所・`RECOVERY.md`・段7への表示 | P0 | 0.5 |
| **P1.7** | **タイムゾーン正規化ルール**の実装 ＋ `AuditLog` | P1 | 0.5 |
| **P2.4** | **計測検証基盤**（合成モニタリング・突合・ボット除外・外れ値検知） | P2.5 | 1 |
| **P4.4** | `ContentVersion` ＋ ロールバック実行 | P4 | 0.5 |
| **P4.8** | 判定の信頼度（対照群最小基準・バッチ判定・inconclusive のUI表現） | P4 | 0.5 |

**追加 3.5日 → 全体 約34.5日**（M-A は 8日に修正）
...
| Mac再起動でサービスが落ちる | Docker restart policy + launchd。段6に最終実行時刻が出るので気づける |
| Threads APIトークン期限切れ（60日） | 自動リフレッシュ＋残日数を段6に常時表示（9日前から黄） |
