  ...
  return (
    <section className="mb-4 rounded-xl border border-[var(--border)] bg-[var(--panel)] p-4">
      ...
      <p className="mb-2 mt-0.5 text-[12px] text-[var(--faint)]">
        ★<strong>記事別には分解できない</strong>。リダイレクタの送り元は設置場所ID
        （<code>media-article-bottom</code> 等）で、どの記事から踏まれたかを持たないため。
        記事別に出すには <code>/r/line/&#123;設置場所&#125;-&#123;記事ID&#125;</code> のように
        記事IDをURLへ入れる必要がある。
      </p>
...
function CoveragePanel({ data }: { data: ContentList }) {
  ...
  const items = [
    { label: "買い手軸", n: c.budget, why: "段3「買い手の質」に要る" },
    { label: "ファネル段階", n: c.funnel, why: "比較段階の流入を出すのに要る" },
  ...
  return (
    <tr className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--panel-2)]">
      ...
      <td className="whitespace-nowrap px-3 py-2.5 text-[12px]">
        <span className={r.budgetTier === "unknown" ? "text-[var(--warn)]" : ""}>
          {BUDGET_LABEL[r.budgetTier] ?? r.budgetTier}
        </span>
      </td>
      <td className="whitespace-nowrap px-3 py-2.5 text-[12px]">
        {r.funnelStage === null ? (
          ...
          (FUNNEL_LABEL[r.funnelStage] ?? r.funnelStage)
        )}
      </td>
    </tr>
  );
