...
const PUBLIC_PATHS = [
  ...
  // ★送客リンク。Threads の投稿本文から訪問者のブラウザが直接踏む。
  //   ログインを要求すると送客が全部止まる。遷移先は環境変数で固定していて
  //   オープンリダイレクトにはならない（app/r/[dest]/[postId]/route.ts）
  // ★末尾に / を付けない。照合が `p` または `${p}/` の前方一致なので、
  //   "/r/" と書くと "/r//..." としか一致せず、公開されないまま気づけない
  "/r",
];
